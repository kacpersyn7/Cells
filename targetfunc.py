...
class Target:
    ...
    def __init__(self, people=np.random.randint(0, 20, size=(g.COLS, g.ROWS)),
                 access_points_types=g.access_points_types, cost_factor=1, people_factor=1, L=300, k=10, x0=10):
        ...
        self.maximum = np.max(self.users_area)
        self.L = L
        self.k = k
        self.x0 = x0
    ...
    def __call__(self, multidimensional_bitmap):
        ...
        target = self.people_factor * np.sum(
            (self.users_area / self.maximum) * sigmoid(new_area, self.L, self.k,
                                                       self.x0)) - self.cost_factor * total_cost
        return target
